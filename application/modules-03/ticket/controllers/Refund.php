...
class Refund extends MX_Controller {
	...
	{ 
		...
		$booking_id = (!empty($getBid)?$getBid:$postBid);
		//  client 2022 project update
		...
		if ($this->form_validation->run()) { 
			...
			$bookinginfo = $this->db->where('id_no',$postBid)->get('tkt_booking')->row();
			$bookby = $bookinginfo->booked_by;
			$userType =  $this->db->where('id',$bookby)->get('user')->row();
			$userTypeEmail = $userType->email;
			
			
			//  client 2022 project update 
				$logUid = $this->session->userdata('id');
				$logInUser =  $this->db->where('id',$logUid)->get('user')->row();
				if ($logInUser->password == md5($this->input->post('password'))) {
            //  client 2022 project update    
                ...
                if ($this->refund_model->create($postData)) {
                    ...
                    if ($userType->is_admin==1) {
                        ...
                        $agent_detail = $this->db->select('*')->from('agent_info')->where('agent_email', $userTypeEmail)->get()->row();
                        
                        $agent_ledger_new = [
                        'agent_id' => $agent_detail->agent_id,
                        'debit' =>  $bookinginfo->amount,
                        'date' => date('Y-m-d H:i:s'),
                        'detail' => "Ticket Refund",
						 // client 2022 project update
                        ...
                        if ($this->input->post('cancelation_fees')>0) {
                            $agent_ledger_new_insert = [
                            'agent_id' => $agent_detail->agent_id,
                            'credit' =>  $this->input->post('cancelation_fees'),
                            'date' => date('Y-m-d H:i:s'),
                            'detail' => "Ticket Cancel Fee",
							 // client 2022 project update
							 'booking_id' => $booking_id,
							 // client 2022 project update
                        ];
                            $this->db->insert('agent_ledger_total', $agent_ledger_new_insert);
                        }
                    }
                    ...
                    if ($this->input->post('cancelation_fees')>0) {
                        $accoutn_transaction = [
                            'account_id' => 60,
                            'transaction_description' => 'Booking ID-' . $postBid . '<br> Ticket Refund ',
                            'amount' => $this->input->post('cancelation_fees'),
	...
	public function passengerInfo(){
		...
		$passengerinfo = $this->db->select('tkt_passenger_id_no')->from('tkt_booking')->where('id_no',$ticket_id)->get()->row();
		$passenger_id = $passengerinfo->tkt_passenger_id_no;
