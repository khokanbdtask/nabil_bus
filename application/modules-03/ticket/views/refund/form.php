        ...
        <div class="panel panel-bd lobidrag">
            ...
            <div class="panel-body">
                ...
                <?= form_open('ticket/refund/form') ?>
                    ...
                    <div class="form-group row">
                        <label for="tkt_passenger_id_no" class="col-sm-3 col-form-label"><?php echo display('passenger_name') ?></label>
                        <div class="col-sm-9">
                             <input name="passenger_name" class="form-control" type="text" placeholder="<?php echo display('passenger_name') ?>" id="passenger_name" value="<?php echo $refund->tkt_passenger_id_no ?>" readonly>
                            <input name="tkt_passenger_id_no" class="form-control" type="hidden" placeholder="<?php echo display('passenger_id') ?>" id="tkt_passenger_id_no" value="<?php echo $refund->tkt_passenger_id_no ?>">
                        </div>
                    </div> 
...
<script type="text/javascript">
    $(document).ready(function() {
     $("#tkt_booking_id_no").on('change',function() {

        //var passengerHelpText = $("#passengerHelpText");

        $.ajax({
            url : '<?php echo base_url('ticket/refund/passengerInfo') ?>',
            method: 'post',  
            dataType: 'json',          
            data: {
            ...
            success: function(data) {
               // passengerHelpText.empty().html(data.name);
                if(data.status == true)
                {
                    $("#tkt_passenger_id_no").val(data.passenger_id);
                    $("#passenger_name").val(data.passenger_name);
                    // swal(data.passenger_name);
                }
                else
                {
                $("#tkt_passenger_id_no").val("");
                $("#passenger_name").val("");
