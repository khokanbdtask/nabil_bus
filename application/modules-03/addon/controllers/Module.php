...
class Module extends MX_Controller
{
    ...
    public function index()
    {
        ...
        if (!empty($live_modules) && isset($live_modules)) {
...
//        $this->form_validation->set_rules('directory', display('module'), 'required|max_length[100]|is_unique[module.directory]');
        $this->form_validation->set_rules('directory', display('module'), 'required|max_length[100]');
...
//                exit();
                ...
                if (($HmvcConfig[$directory]["_database"] === true) && !empty($HmvcConfig[$directory]["_tables"]) && is_array($HmvcConfig[$directory]["_tables"]) && sizeof($HmvcConfig[$directory]["_tables"]) > 0) {
                    ...
                    if (strlen($error) > 0) {
                        ...
                        /// IF MODULE ALREADY EXIST IN MODULE TABLE, THEN JUST UPDATE THE STATUS

                        if ($this->module_model->check_module_exist($directory) == 0) {
                            if($this->module_model->update_module_status($directory, 1))
                            {
                                // add a install flag
                                @file_put_contents(
    ...
    public function uninstall($dirPath = null, $action = null)
    {
        ...
        if (is_dir($dirPath) && $dirPath != $basePath) {
            ...
            ////////////////////////////////////////////
            // Data delete into existing table /////////
            ////////////////////////////////////////////

            if ($HmvcConfig[$directory]["_uninstall_query"] === true) {
                $uninstall_query = APPPATH . 'modules/' . $directory . '/assets/data/uninstall.sql';
                if (file_exists($uninstall_query) && file_exists($configPath)) {

                    $equery_file = file_get_contents($uninstall_query);
                    $equery_string = trim($equery_file);
            ...
            if ($action == "delete") {
                ...
                $this->module_model->module_purchase_key($table,$update_date);
            ...
            if ($result->status == 'valid') {
                ...
                if ($copydata) {
                    ...
                    if ($unzip) {
                        ...
                        $m_p_data = array(
                            'module_identity' => $result->identity,
