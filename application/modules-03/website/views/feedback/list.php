                            ...
                            <tr>
                                ...
                                <td>
                                    <?php if ($this->permission->method('website', 'update')->access()): ?>
                                        <input type="checkbox" id="add_to_website" name="add_to_website"
                                               data-id="<?php echo $feedback->id ?>"
                                               value="<?php echo(($feedback->add_to_website == 1) ? '0' : '1') ?>" <?php echo(!empty($feedback->add_to_website) ? 'checked' : null) ?>>
                                    <?php endif; ?>
                                </td>
                                <td>

                                    <?php if ($this->permission->method('website', 'delete')->access()): ?>
                                        <a href="<?php echo base_url("website/feedback/delete/$feedback->id") ?>"
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        // $("#add_to_website").change(function () {
        addToWeb.change(function () {
            var x = 0;
            $.ajax({
                url: '<?php echo base_url('website/feedback/update') ?>',
                method: 'post',
                dataType: 'json',
                data:
                    {
