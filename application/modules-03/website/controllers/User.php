...
class User extends MX_Controller
{
    ...
    public function user_details()
    {
        ...
        $userid  = $this->session->userdata('id_no');
        $this->db->where('receive_id',   $userid);
        $this->db->order_by("id","desc");
        $result = $this->db->get('luggage_partial_payment');
        $data['luggage'] = $result->result();
         // New code 2021 direct update 
        ...
        $this->load->view('website/' . $this->active_theme . '/user/user_details', $data);
    ...
    public function view($id = null)
    {
        ...
        // print_r($data);
        // exit();
        $this->load->view('website/' . $this->active_theme . '/user/ticket', $data);
    }
    ...
    public function upform_pasenger($id = null)
    {
        ...
        // print_r($data);
        // exit();
        $this->load->view('website/' . $this->active_theme . '/user/passenger_upform', $data);
...
// user bank and payment information
            ...
            if (!empty($this->input->post('nid'))) {
                if ($this->website_model->email_check($this->input->post('email'))) {
                    ...
                    if ($this->website_model->createPassenger($postData)) {
                        $data['status'] = true;
                        $data['success'] = 'successfully Registerd';
                ...
                $data['status'] = false;
                $data['exception'] = 'Please input Your NID NO';
            }
    ...
    // New code 2021 direct update
    public function successPaymentPartial($bookingid)
    {
        $data = array(
            'payment_status' => "paid",
         );
         $this->db->where('luggage_booking_id',$bookingid);
        $this->db->update('luggage_partial_payment', $data);
       $this->user_details();
    }
    // New code 2021 direct update
