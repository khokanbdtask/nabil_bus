    ...
    */
    ...
    public function paymentlocal($booking_id_no = null)
    {
       ...
       $tax_modules = array();
        ...
        $tax_module = $this->tax_model->tax_setting_info();
    ...
    public function ozow()
    {
      ...
      if($status == "Complete")
      {
       ...
       $tax_modules = array();
        ...
        $output = $dompdf->output();
        //file_put_contents('assets/data/pdf/' . $id . '.pdf', $output);
        file_put_contents('ticket.pdf', $output);
        ...
        $accoutn_transaction = [
            'account_id' => 3,
            'transaction_description' => 'Booking ID-' . $ticket->id_no . '<br> Route ID-' . $ticket->trip_route_id . '<br> Trip Id-' . $ticket->trip_id_no . '' . '<br> Ticket No-' . $ticket->seat_numbers . '' . '<br> Passanger ID-' . $ticket->tkt_passenger_id_no . '' . '<br> Seat Price-' .$ticket->price . '' . '<br> Discount-' . $ticket->discount . '',
            'amount' => ((($ticket->price*$total_tax_rate)/100)+$ticket->price),
            'create_by_id' => null,
