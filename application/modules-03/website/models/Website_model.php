...
public function read_setting()
	{
		...
		return $this->db->query("
			SELECT ws_setting.* , setting.logo AS slogo, setting.footer_text,setting.language 
...
public function getlocal($booking_id_no = null)
    ...
    */ 
    ...
    public function passenger_email($id='')
    {
  ...
  //           $this->db->insert('ticket_notification',$notice);
    ...
    public function getLuggage($booking_id_no = null)
    {
        $rowData = Array();
...
//        }
        ...
        return $this->db->select("
                lb.pickup_trip_location,
                lb.drop_trip_location,
                CONCAT_WS(' ', tp.firstname, tp.lastname) AS passenger_name, 
                lb.luggage_passenger_id_no AS luggage_passenger_id_no,
                lb.trip_id_no AS trip_id_no,
                lb.id_no AS booking_id_no,
                DATE_FORMAT(lb.booking_date, '%m/%d/%Y %h:%i %p') as booking_date,
                tr.name AS route_name, 
                lb.amount AS amount,
                lb.package_id AS package_id,
                lb.discount AS discount,
                lb.booking_type,
                lb.trip_route_id,
                lb.payment_status,
                tp.nid
            ...
            ->join('tkt_passenger AS tp', 'lb.luggage_passenger_id_no = tp.id_no' ,'left')
            ->join('trip_route AS tr', 'tr.id = lb.trip_route_id','left')
            ->where('lb.id_no', $booking_id_no)
            ->get()
            ->row();
    }
...
public function location_name($id='')
    {
        return $loc_name = $this->db->select('id,name')
            ->from('trip_location')
            ->where('id', $id)
            ->get()
            ->row();
    ...
    {
        $themes = null;
        if($this->db->count_all('themes') > 0)
        {
            $themes = $this->db->select('directory')
                    ->from('themes')
                    ->where('status',1)
                    ->get()
                    ->row()
        ...
        return ($themes)?$themes:null;
    }

    public function seat_available($trip_id_no,$b_date)
    {
        // Code Here

        return $this->db->select("SUM(tb.total_seat) AS available")
            ->from("tkt_booking AS tb")
    ...
    public function total_book_seat($trip_route_id,$trip_id_no,$booking_date)
    {
      ...
      $oldquery  = $oldquery->row();             
        $query = $this->db->select('*')
                ->from("tkt_booking")
                ->where('trip_id_no', $oldquery->id)
                ->where('trip_route_id', $trip_route_id)
