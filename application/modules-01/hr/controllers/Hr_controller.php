...
class Hr_controller extends MX_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->permission->module('hr')->redirect();
    ...
    public function create_hr()
    {
        ...
        #-------------------------------#
        $this->form_validation->set_rules('first_name', display('first_name'), 'trim|required|max_length[50]');
        $this->form_validation->set_rules('second_name', display('second_name '), 'trim|required|max_length[200]');
        $this->form_validation->set_rules('position', display('position'), 'trim|required|max_length[50]');
        $this->form_validation->set_rules('phone_no', display('phone_no '), 'trim|required|max_length[200]');
        $this->form_validation->set_rules('email_no', display('email_no '), 'trim|max_length[200]');
        $this->form_validation->set_rules('document_id', display('document_id '), 'trim|max_length[200]');
        $this->form_validation->set_rules('address_line_1', display('address_line_1'), 'trim|max_length[50]');
        $this->form_validation->set_rules('address_line_2', display('address_line_2 '), 'trim|max_length[200]');
        ...
        $doc = $this->myupload->do_upload(
            './application/modules/hr/assets/images/',
            'document_pic');
        $this->form_validation->set_rules('blood_group', display('blood_group'), 'trim|max_length[20]');

        $this->form_validation->set_rules('country', display('country '), 'trim|max_length[30]');

        $this->form_validation->set_rules('city', display('city'), 'trim|max_length[50]');
        $this->form_validation->set_rules('zip', display('zip '), 'trim|max_length[20]');
        ...
        if ($this->form_validation->run() === true) {
            ...
            $postData = [
                ...
                'picture' => (!empty($img) ? $img : './application/modules/hr/assets/images/pic.png'),
                'document_pic' => $doc,
            ...
            if ($this->hr_model->hr_create($postData)) {
                ...
                // 'closed_by_id' => 'null'
                // ];
                // if($this->input->post('position',true) =='Driver'){
                //    // print_r($driverinfo);exit;
                //  $this->db->insert('driver_history',$driverinfo);
                // }
                // if($this->input->post('position',true) =='Assistant'){
                //    // print_r($driverinfo);exit;
                //  $this->db->insert('assistant_history',$assistant_info);
                // }
    ...
    public function hr_update($id = null)
    {
        ...
        if ($this->form_validation->run() === true) {
            ...
            if ($this->hr_model->update_hr($Data)) {
                if ($this->input->post('position', true) == 'Driver') {
                    $this->db->where('driver_id', $this->input->post('id', true))
                        ->update('driver_history', $driverinfo);

                }
                if ($this->input->post('position', true) == 'Assistant') {
                    $this->db->where('assistant_id', $this->input->post('id', true))
                        ->update('assistant_history', $assistant_info);
                }
