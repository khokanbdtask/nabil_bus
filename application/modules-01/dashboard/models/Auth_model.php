...
// password recovery
    ...
    //Passenger Logged in or not - Check
    public function passenger_check($id_no)
    {
        return $this->db->select("*")
            ->from('tkt_passenger')
            ->where('id_no', $id_no)
	...
	//Passenger password recovery
		public function passenger_password_recovery($data = array())
	{
		return $this->db->select("*")
			->from('tkt_passenger')
			->where('email', $data['email'])
...
public function update_forgot_pass($data = [])
	{
		return $this->db->where('password_reset_token',$data['password_reset_token'])
			->update('user',$data); 
	}

	// passernger

public function update_passenger_forgot_pass($data = [])
	{
		 if($this->db->where('password_reset_token',$data['password_reset_token'])
	...
	// update and check passenger email
