...
//             }
//             elseif (!empty($this->input->post('hdn_passenger_id'))) {
//                $passengerid = $this->input->post('hdn_passenger_id');
//              }
//             else {
...
//             }
     ...
     // die();
            ...
            $data['pickup'] = $this->website_model->location_by_id($binfo->pickup_trip_location);
            $data['drop'] = $this->website_model->location_by_id($binfo->drop_trip_location);
...
//        echo json_encode($data);
    ...
    */
    ...
    public function checkout()
    {
        ...
        $this->form_validation->set_rules('firstname', display('firstname'), 'trim|required|max_length[50]');
        $this->form_validation->set_rules('lastname', display('lastname'), 'trim|required|max_length[50]');
        $this->form_validation->set_rules('phone', display('phone'), 'trim|required|max_length[20]');
        $this->form_validation->set_rules('email', display('email'), 'trim|required|valid_email|max_length[100]|is_unique[tkt_passenger.email]');
        $this->form_validation->set_rules('password', display('password'), 'max_length[100]|trim');
        ...
        $passenger_id = (empty($this->input->post('hdn_passenger_id')))?$this->randdomString("P"):$this->input->post('hdn_passenger_id');
        ...
        $data['tkt_passenger_id_no'] = $passenger_id;
            ...
            if ($this->website_model->email_check($this->input->post('email'))) {
                ...
                if ($this->website_model->createPassenger($postData)) {
                    ...
                    if (!empty($passenger_id)) {
                        $this->db->where('id_no', $binfo->id_no);
                        $this->db->update('ws_booking_history', ["tkt_passenger_id_no" => $passenger_id ]);
                    }
    ...
    public function bank_booking()
    {
        ...
        if ($this->form_validation->run()) {
            ...
            if ($this->website_model->createbanktransaction($bankdata)) {
                ...
                // redirect
                redirect(base_url('website/paypal/bank_info/' . $this->input->post('booking_id_no')),'location');
            } else {
                $data['status'] = false;
                $data['exception'] = display('please_try_again');
            }
