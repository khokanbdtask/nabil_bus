...
//        $facilities = Array();
        ...
        $this->form_validation->set_rules('total_weight',display('total_weight')  ,'required|max_length[11]');
		$this->form_validation->set_rules('status',display('status') ,'required');
		$this->form_validation->set_rules('luggage_service',display('luggage_service') ,'required');
		$facilities = $this->input->post('fleet_facilities');
		$facilities = implode(',' , (array) $facilities);
		#-------------------------------#
		$data['fleet_type'] = (Object) $postData = [
			...
			'luggage_service' => $this->input->post('luggage_service'),
		];
