...
class Agent_controller extends MX_Controller {
    ...
    {  
      ...
      $this->form_validation->set_rules('agent_commission',display('agent_commission'),'required|max_length[50]'); 
        ...
        // New code 2021 direct update
        $data['location'] = $this->db->get('trip_location')->result();
        // New code 2021 direct update 
    ...
    // New code 2021 direct update 
    public function agent_ledger_new($id = null){
        $this->permission->method('agent','read')->redirect();
        ...
        {
            $data['ledger'] = $this->agent_model->agent_ledger_new($id,$dateform,$dateto );
        }
        else
        {
            $data['ledger'] = $this->agent_model->agent_ledger_new($id);
        }
