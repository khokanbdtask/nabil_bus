...
class Theme extends MX_Controller {
  ...
  private $theme_location = APPPATH.'modules/website/views/';
...
//            echo Modules::run('theme/list', $data);
//              $this->template_lib->full_admin_html_view($content);
        ...
        $data['page'] = "theme/list";
         ...
         echo $this->parser->parse('template/layout', $data, true);

//        echo "<pre>";
//        print_r($content);
//        exit();

//        echo Modules::run('template/layout', $content);
...
//          if(!empty($response) && ($response->status == 'valid')) {
            ...
            $target_dir = 'application/modules/website/views';
            
            // naming for theme
            if ($theme_name !== '') {
              $dir = strtolower($theme_name);
              $target_dir = 'application/modules/website/views/';
            }
            ...
            $config = array();
            $config['upload_path'] = './application/modules/website/views/';
          ...
          $this->load->library('upload');
            ...
            if (!$this->upload->do_upload('new_theme')) {
                ...
                if ($x === true) {
                    ...
                    chmod($target_dir .'/', 0777); //change uploaded file permission
                    $this->themes_model->store($dir,$name[0]); //insert store name into database
          ...
          if ($result->status == 'valid') {
            ...
            if($copydata) 
            {
              ...
              if($unzip) {
                ...
                $tdata = array(
                  'name' => $result->title,
                  'directory' => $result->identity,
                  'description' => $result->description,
                  'download_details' => "Downloaded from API at ".date('Y-m-d H:i:s')." and Download URL is ".$result->download_url,
                  'status' => 0
                );
