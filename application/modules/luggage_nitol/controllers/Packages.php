...
class Packages extends MX_Controller
{
    ...
    public function create_price()
    {
        ...
        $this->form_validation->set_rules('route_id[]', display('route_id'), 'required');
        ...
        $route_id = $this->input->post('route_id[0]');
        ...
        if ($this->form_validation->run()) {
            if(empty($checkPackage))
            {
                ...
                $routIds =  $this->input->post('route_id[]');
                ...
                if ($lpm = $this->Packages_model->batchCreatePackage($postData)) {
...
//       $price_info = $this->db->select('*')->from('pri_price')->where('route_id',$route_id)->where('vehicle_type_id',$type)->get();
    ...
    public function luggageOtherLocation()

    {
        $this->permission->module()->read()->redirect();
        $data['title'] = "Other Location List";
        #-------------------------------#
        $data['otherlocation'] = $this->Packages_model->getAllOtherLocation();
        ...
        $data['module'] = "luggage_nitol";
        $data['page'] = "location/other_location_list";
    ...
    public function otherLocationFromLoad()
    {
        ...
        $data['route_list'] = $this->Packages_model->rout();
        
        $data['module'] = "luggage_nitol";
        $data['page'] = "location/other_location_form";
    ...
    public function addOtherLocation()
    {
                ...
                else
                {
                    ...
                    if ($this->Packages_model->otherLocationcreate($data)) {
                        $this->session->set_flashdata('message', display('save_successfully'));
                    } else {
                        $this->session->set_flashdata('exception', display('please_try_again'));
                    }
                        redirect("luggage_nitol/packages/luggageOtherLocation");
                }
    ...
    public function otherLocationEdit($otherLocationID)
    {
        ...
        $data['route_list'] = $this->Packages_model->rout();
        $data['otherlocation'] = $this->Packages_model->getOtherLocation($otherLocationID);
        ...
        $data['module'] = "luggage_nitol";
        $data['page'] = "location/other_location_form_edit";
        echo Modules::run('template/layout', $data);
        ...
        if ($this->form_validation->run() === FALSE)
                {
                    ...
                    $data['title'] = "Other Location Edit";
                    $data['route_list'] =  $this->Packages_model->rout();
                    $data['otherlocation'] =$this->Packages_model->getOtherLocation($locationId);
                ...
                else
                {
                    ...
                    if ($this->Packages_model->updateOtherLocation($locationId,$data)) {
                        $this->session->set_flashdata('message', display('save_successfully'));
                    } else {
                        $this->session->set_flashdata('exception', display('please_try_again'));
                    }
    ...
    {
        $data = $this->Packages_model->getOtherLocationData($roteId);
