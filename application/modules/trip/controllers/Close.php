...
class Close extends MX_Controller
{
    ...
    public function form($id = null)
    {
        ...
        $driver_id = $this->db->select('*')->from('trip_assign')->where('id', $id)->get()->row();
        #-------------------------------#
        $postData = [
        ...
        if ($this->form_validation->run()) {
            ...
            if (!empty($postData['id'])) {
                ...
                if ($this->assign_model->update($postData)) {
                    $this->db->where('id', $driver_id->driver_id)
                        ->update('employee_history', $driverinfo);
                    $this->db->where('id', $driver_id->fleet_registration_id)
                        ->update('fleet_registration', $fleetinfo);
                    $this->db->where('id', $driver_id->assistant_1)
                        ->update('employee_history', $assistant1);
                    $this->db->where('id', $driver_id->assistant_2)
                        ->update('employee_history', $assistant2);
                    $this->db->where('id', $driver_id->assistant_3)
            ...
            $data['close_arr'] = (array)$this->assign_model->findById($id);

            $data['luggage_booking_list'] = $this->assign_model->luggage_booking_list($data['close_arr']['trip_assign_id']);
