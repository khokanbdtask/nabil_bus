...
class Assign_model extends CI_Model
{
    ...
    {
        return $this->db->select("a.*,a.status AS trip_assign_status,b.reg_no,c.*,CONCAT_WS(' ', d.first_name, d.second_name) AS driver_name,CASE WHEN a.closed_by_id THEN 'bg-success' ELSE NULL END AS isClosed,e.name AS trip_route_name")
            ->from("trip_assign a")
    ...
    public function findByIdNo($id_no = null)
    {
		...
		")->from("trip_assign AS t")
            ->join('tkt_booking AS tb', 'tb.trip_id_no = t.id', 'left')
            ->join('luggage_booking AS lb', 'lb.trip_id_no = t.id', 'left')
    ...
    public function luggage_booking_list($id='')
    {
        $luggage = (array)$this->db->select("COUNT(id_no) AS total_lb, SUM(amount) AS total_lb_income")
                        ->from("luggage_booking")
                        ->where("trip_id_no",$id) 
                        ->where("payment_status",'NULL') 
                        ->where("delete_status",0)
                        ->where("luggage_refund_id",null)
                        ->get()
                        ->row();

        $ticket = (array)$this->db->select("COUNT(id_no) AS total_tb, SUM(amount) AS total_tb_income")
                        ->from("tkt_booking")
                        ->where("trip_id_no",$id)
                        ->where("payment_status",'NULL') 
                        ->or_where("payment_status",'') 
                        ->where("booking_delete_status",0)
                        ->where("tkt_refund_id",null)
                        ->get()
                        ->row();

        $total_booking = $luggage['total_lb']+$ticket['total_tb'];
        $total_income = $luggage['total_lb_income']+$ticket['total_tb_income'];

        return $data = array("total_booking" =>$total_booking, "total_income" => $total_income);
...
// insert shedule
    ...
    public function dropdowndriver()
    {
    ...
    public function dropdownassistant()
    {
        $data = $this->db->select("id, CONCAT_WS(' ',first_name, second_name) AS name")
            ->from("employee_history")
            ->where('position', 'Assistant')
    ...
    public function driverdropdownupdate($id)
    {

        $data = $this->db->select("employee_history.id, CONCAT_WS(' ',first_name, second_name) AS name")
            ->from("employee_history")
            ->join('dynamic_assign', 'dynamic_assign.employeeid = employee_history.id')
            ->where('dynamic_assign.randomid', $id)
    ...
    public function driverdropdownupdateNodynamic($id)
    {

        $data = $this->db->select("employee_history.id, CONCAT_WS(' ',first_name, second_name) AS name")
            ->from("employee_history")
            ->join('trip_assign', 'trip_assign.driver_id = employee_history.id')
            ->where('trip_assign.id_no', $id)
            ->get()
            ->result();
    ...
    public function asistnsedropdownupdate($id)
    {
        ...
        $data = $this->db->select("employee_history.id, CONCAT_WS(' ',first_name, second_name) AS name")
            ...
            ->join('dynamic_assign', 'dynamic_assign.employeeid = employee_history.id')
            ->where('dynamic_assign.randomid', $id)
            ->where('dynamic_assign.employeetype', 'Assistant')
            ->get()
            ->result();
    ...
    public function asistantdropdownupdateNodynamic($id)
    {

        $data = $this->db->select("employee_history.id, CONCAT_WS(' ',first_name, second_name) AS name")
            ->from("employee_history")
            ->join('trip_assign', 'trip_assign.assistant_1 = employee_history.id')
            ->where('trip_assign.id_no', $id)
            ->get()
            ->result();
    ...
    public function asistantUpdate($id)
    {
