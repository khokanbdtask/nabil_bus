...
class Trip_model extends CI_Model
{
    ...
    {
        return $this->db->insert_batch("other_location", $data);
    }


    public function routListdropdown()
    {
        $data = $this->db->select("*")
            ->from('trip_route')
            ->where('status', 1)
    ...
    {
        return $this->db->select("other_location.*,other_location.id as otherlocationid,other_location.status as otherstatus,trip_route.*")
            ->from('other_location')
            ->join('trip_route', 'other_location.trip_route_id = trip_route.id')
    ...
    public function updateOtherLocation($otherLocationId,$upDateData)
    {
        return $this->db->where('id', $otherLocationId)
            ->update('other_location', $upDateData);
    }
