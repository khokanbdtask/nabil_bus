                    ...
                    <?php if (!empty($hr)) { ?>
                        ...
                        <?php foreach ($hr as $query) { ?>
                            <tr class="<?php echo ($sl & 1) ? "odd gradeX" : "even gradeC" ?>">
                                ...
                                <td class="center">
                                    <?php if (!empty($query->document_pic)) { ?>
                                        <a href="<?php echo base_url("hr/hr_controller/download?file=$query->document_pic") ?>"
                                           tareget="_blank" class="btn btn-xs btn-info"><i
                                                    class="fa fa-download"></i></a>
                                    <?php } ?>
                                    <?php if ($this->permission->method('hr', 'read')->access()): ?>
                                        <a href="<?php echo base_url("hr/hr_controller/emp_details/$query->id") ?>"
                                           class="btn btn-xs btn-default"><i class="fa fa-eye" aria-hidden="true"></i>
                                        </a>
                                    <?php endif; ?>

                                    <?php if ($this->permission->method('hr', 'update')->access()): ?>
                                        <a href="<?php echo base_url("hr/hr_controller/hr_update/$query->id") ?>"
                                           class="btn btn-xs btn-success"><i class="fa fa-pencil"></i></a>
                                    <?php endif; ?>

                                    <?php if ($this->permission->method('hr', 'delete')->access()): ?>
                                        <a href="<?php echo base_url("hr/hr_controller/hr_delete/$query->id") ?>"
