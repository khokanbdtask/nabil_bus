...
// update terms and condition
    ...
    public function luggage_report($datefrom = "", $dateto = "", $ftypes = "", $route_id = "")
    {
        

        if (empty($ftypes) && !empty($route_id)) {

            return $this->db->select("luggage_booking.*, trip_route.name AS route_name, package.package_name AS package_name")
            ->from('luggage_booking')
            ->join("trip_route", "trip_route.id = luggage_booking.trip_route_id", "left")
            ->join('package', 'package.package_id = luggage_booking.package_id','left')
            ->where('luggage_booking.trip_route_id', $route_id)
            ->where('luggage_booking.booking_date >=', $datefrom)
            ->where('luggage_booking.booking_date <=', $dateto)
            ->get()
            ->result();
            ...
            ->join("trip_route", "trip_route.id = luggage_booking.trip_route_id", "left")
            ->join('package', 'package.package_id = luggage_booking.package_id','left')
            ->where('luggage_booking.ftypes', $ftypes)
            ->where('luggage_booking.booking_date >=', $datefrom)
            ->where('luggage_booking.booking_date <=', $dateto)
            ->get()
            ->result();
